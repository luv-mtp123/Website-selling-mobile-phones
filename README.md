...
* **✅ Nâng Cấp Toàn Diện AI Smart Search (Hybrid Search):**
    * **Hybrid Search:** Kết hợp hoàn hảo giữa SQL (lọc chính xác giá, hãng) và Vector DB (hiểu ngữ nghĩa từ lóng "pin trâu", "củ").
    * **Advanced Prompt Engineering:** Dạy AI quy đổi tiền tệ ("củ" -> số 0), tự sửa lỗi chính tả và phân loại chuẩn phụ kiện.
...
* **✅ Chatbot Tư Vấn:** Trả lời tự động các câu hỏi thường gặp (FAQs) và đóng vai trò chuyên gia tư vấn lựa chọn sản phẩm phù hợp.
* **✅ Chatbot Memory (MỚI):** Tích hợp khả năng ghi nhớ lịch sử hội thoại ngắn hạn (Contextual Awareness). AI có thể hiểu các đại từ thay thế như "nó", "cái đó", "loại này" trong các câu hỏi nối tiếp của người dùng. 
...
## 29. 🧪 Kịch Bản Kiểm Thử Bẫy Ngữ Nghĩa (Search AI Test Cases)

Dưới đây là 5 "Bẫy Ngữ Nghĩa" và "Bẫy Hệ Thống" được thiết kế để tự tay kiểm chứng sức mạnh của thanh tìm kiếm **Hybrid Search AI**:

* **🚨 Test Case 1: Bẫy Phụ Kiện "Đội Lốt" Điện Thoại**
    * **Mô tả:** Khách hàng gõ tên dòng điện thoại cao cấp nhưng thực chất chỉ muốn mua cái ốp lưng.
    * **Input:** `ốp lưng xịn cho iphone 15 pro max` | **Dropdown:** Để trống.
    * **Kết quả kỳ vọng:** Cột thông báo hiện 🤖 AI đã phân tích nhu cầu | Hãng: Apple. Chỉ hiển thị các loại phụ kiện Apple (Ốp Lưng MagSafe, Silicon). Tuyệt đối không hiển thị điện thoại iPhone 15.

---

* **🚨 Test Case 2: Bẫy Từ Lóng Tiền Tệ & Giữ Nguyên Tính Năng**
    * **Mô tả:** Khách dùng ngôn ngữ mạng ("củ", "cày game") và yêu cầu hệ thống giữ nguyên từ khóa "game".
    * **Input:** `máy samsung nào cày game tầm 15 củ quay đầu` | **Dropdown:** Để trống.
    * **Kết quả kỳ vọng:** Tự động lọc các điện thoại Samsung ≤ 15.000.000 VNĐ. Các máy đắt tiền (S24 Ultra) phải biến mất. Chữ "game" được giữ lại giúp Vector AI ưu tiên các máy hiệu năng cao.

---

* **🚨 Test Case 3: Bẫy Chồng Chéo Bộ Lọc (AI + Dropdown)**
    * **Mô tả:** Khách hàng bắt AI phân tích tính năng nhưng đồng thời ép cứng Hãng sản xuất tại Dropdown.
    * **Input:** `điện thoại chụp ảnh ban đêm cực kỳ đẹp` | **Dropdown:** Chọn Samsung.
    * **Kết quả kỳ vọng:** Hệ thống không được bỏ rơi AI. Chỉ hiển thị điện thoại Samsung. Các máy có thông số "Camera 200MP", "Chụp đêm cực đỉnh" phải được đẩy lên vị trí đầu tiên.

---

* **🚨 Test Case 4: Bẫy SQL Fallback (Đọc Cột Mô Tả)**
    * **Mô tả:** Kiểm tra thuật toán SQL thuần túy khi tìm kiếm tính năng chỉ nằm trong phần Mô tả sản phẩm.
    * **Input:** `leica` hoặc `chống ố vàng` | **Dropdown:** Để trống.
    * **Kết quả kỳ vọng:** Gõ `leica` trả về Xiaomi 14; gõ `chống ố vàng` trả về Ốp Lưng MagSafe. Thuật toán dự phòng phải quét qua cả cột `Description` thay vì chỉ quét `Name`.

---

* **🚨 Test Case 5: Bài Test Chế Độ "Bất Tử" (Offline Vector DB)**
    * **Mô tả:** Giả lập trường hợp Google AI cạn Quota hoặc mất kết nối mạng.
    * **Chuẩn bị:** Vô hiệu hóa `GEMINI_API_KEY` trong file `.env` và F5 website.
    * **Input:** `mua củ sạc nhanh` | **Dropdown:** Để trống.
    * **Kết quả kỳ vọng:** Website không văng lỗi 500. Bộ não Vector Offline (`sentence-transformers`) vẫn hoạt động trên CPU local và trả về kết quả củ sạc chuẩn xác.
