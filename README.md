...
* **✅ Tối Ưu Tốc Độ & Quota:** Kích hoạt bảng `AICache` trong CSDL để tự động lưu trữ các câu trả lời của AI. Nếu khách hàng hỏi một câu trùng lặp, hệ thống sẽ truy xuất DB để phản hồi siêu tốc trong < 0.1 giây thay vì chờ API Google từ 3-5 giây, giúp tiết kiệm 100% hạn mức Quota.
...
* **✅ Quản Trị Vector AI (Vector Manager):** Refactor toàn bộ logic ChromaDB thành class `vector_manager.py` chuyên biệt, mã hóa ngôn ngữ tự nhiên thành Vector đa chiều (768 chiều) với model Google Embedding chuẩn xác.
...
| :--- | :--- | :--- |
| **01** | **Cài đặt thư viện** | `py -m pip install -r requirements.txt` |
| **02** | **Cấu hình hệ thống** | Tạo file `.env` tại thư mục gốc và điền đầy đủ: `Gemini_API_Key`, `Google_OAuth`, `Secret_Key`. |
