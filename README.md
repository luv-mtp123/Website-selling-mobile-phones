...
* **✅ Động Cơ Tìm Kiếm Bằng Hình Ảnh (Visual AI Search):**
	* **Kiến trúc Lai (Hybrid Vision):** Kết hợp hoàn hảo giữa Sức mạnh Đa phương thức (Gemini Vision) và Trích xuất Đặc trưng (MobileNetV2 - PyTorch).
	* **Đọc Hiểu Tên Máy:** Khi khách tải ảnh lên, AI Gemini Vision sẽ làm nhiệm vụ "nhìn" và đọc đích danh Tên máy + Hãng sản xuất (VD: "Apple iPhone 15 Pro Max") thay vì chỉ dùng thuật toán Vector cảm nhận hình khối chung chung.
	* **Cơ chế Lọc Hãng (Brand Isolation):** Giải quyết triệt để lỗi "Tìm Xiaomi ra Samsung". Hệ thống sẽ ưu tiên hiển thị chính xác dòng máy được nhận diện. Nếu kho hết hàng, hệ thống mới gọi đến MobileNetV2 (Vector Hình Dáng) để tìm các máy có "Kiểu dáng tương đồng", và bắt buộc phải lọc qua bộ lọc Hãng để đảm** bảo gợi ý máy cùng hệ sinh thái.
	...
	* **Tối ưu VRAM Server:** Mô hình MobileNetV2 được chạy ở chế độ dự đoán eval() siêu nhẹ, phối hợp nhịp nhàng với sức mạnh của Gemini API, không gây áp lực tải lên Server web.
