...
* **✅ Động Cơ Tìm Kiếm Bằng Hình Ảnh (Visual AI Search - Nâng cấp V3):**
	* **Kiến trúc Lai (Hybrid Vision):** Kết hợp hoàn hảo giữa Sức mạnh Đa phương thức (Gemini Vision) và Trích xuất Đặc trưng Không Gian Sâu (Nâng cấp từ MobileNetV2 lên ResNet-50 - PyTorch).
	* **Nhận Diện Cấp Độ Chuyên Gia (Fine-Grained Master):** Bổ sung bộ quy tắc mổ xẻ thiết kế siêu vi (Chain-of-Thought) cho AI. Bắt buộc AI soi bản lề (Fold gapless), chất liệu viền (Titan/Thép), hình dáng camera (chữ U, viên thuốc) để phân biệt chính xác 100% các đời máy giống nhau của Samsung và iPhone.
	* **Cơ chế Lọc Hãng (Brand Isolation):** Giải quyết triệt để lỗi "Tìm Xiaomi ra Samsung". Hệ thống sẽ ưu tiên hiển thị chính xác dòng máy được nhận diện. Nếu kho hết hàng, hệ thống mới gọi đến ResNet-50 (Vector Hình Dáng 2048 chiều) để tìm các máy có "Kiểu dáng tương đồng", và bắt buộc phải lọc qua bộ lọc Hãng để đảm** bảo gợi ý máy cùng hệ sinh thái.
	* **Tăng Tốc Phần Cứng (Hardware Acceleration):** Tự động nhận diện Card đồ họa (NVIDIA CUDA hoặc Apple MPS) để đẩy mô hình ResNet-50 lên GPU, giúp tốc độ trích xuất Vector tăng gấp 10 - 50 lần so với chạy CPU thuần.
	* **Chống Ảo Giác & Tối Ưu Băng Thông (Anti-Hallucination & Compression):** Tự động nén ảnh (Resize < 1024px) giúp tiết kiệm 80% băng thông API, chống lỗi nghẽn mạng. Bổ sung khiên bảo vệ bắt buộc AI TỪ CHỐI phân tích nếu ảnh tải lên không phải là thiết bị di động (động vật, phong cảnh).
