...
* **✅ Động Cơ Tìm Kiếm Bằng Hình Ảnh (Visual AI Search):**
	* **Kiến trúc:** Tích hợp mô hình Computer Vision MobileNetV2 (PyTorch) siêu nhẹ. Khi Admin thêm sản phẩm, ảnh được AI "nhìn" và trích xuất thành mảng đặc trưng (Feature Extractor - 1280 chiều) và lưu vào ChromaDB với thuật toán Cosine Similarity.
	* **Cơ chế hoạt động:** Khách hàng không cần gõ chữ miêu tả dài dòng, chỉ cần bấm nút 📷 và tải ảnh chụp thiết bị lên. Hệ thống sẽ ngay lập tức biến ảnh đó thành Vector, so khớp với kho ảnh trong CSDL và trả về sản phẩm có ngoại hình giống nhất.
	* **Tại sao Tương đồng là Thành công?** AI Visual Search không hoạt động rập khuôn như máy quét mã vạch (Barcode), nó hoạt động dựa trên việc "cảm nhận" hình khối tổng thể, viền màn hình và số lượng camera. Việc AI trả về đúng sản phẩm, hoặc các sản phẩm có thiết kế cực kỳ tương đồng (VD: Phân vân giữa iPhone 14 Pro Max vs 15 Pro Max) chứng tỏ hệ thống thần kinh nhân tạo của AI đang xử lý góc cạnh cực chuẩn xác.
	* **Tối ưu VRAM Server:** Mô hình được chạy ở chế độ dự đoán eval() (zero-shot, khai thác trí thông minh đã được đào tạo sẵn trên hàng triệu bức ảnh ImageNet) mà không cần phải Train lại, đảm bảo Server web siêu nhẹ và không bao giờ bị sập.
